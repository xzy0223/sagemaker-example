...
def get_train_db(train_data_path):
    # 获取train data路径下的所有文件
    files = os.listdir(train_data_path)
    # 存放pandas dataframe的列表
    dfs = []
    
    # 遍历各个文件，生成data frame并存入列表
    for f in files:
        dfs.append(pd.read_csv(os.path.join(train_data_path, f), sep=',',header=None))
    
    # 合并各个data frame
    conbined_df=pd.concat(dfs, axis=0, ignore_index=True)
    
    # 分别获得feature和label
    x_train, y_train = conbined_df.values[:, 1:], conbined_df.values[:, 0]
    
    # 生成tensorflow training dataset
...
def get_validation_db(validation_data_path):
    ...
    for f in files:
        dfs.append(pd.read_csv(os.path.join(validation_data_path, f), sep=',',header=None))
        
    conbined_df = pd.concat(dfs, axis=0, ignore_index=True)
    
    x_validation, y_validation = conbined_df.values[:, 1:], conbined_df.values[:, 0]
